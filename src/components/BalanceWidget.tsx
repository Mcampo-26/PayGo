interface BalanceProps {
  ...
  export const BalanceWidget = ({ amount, status }: BalanceProps) => {
    ...
    // 💡 Mejora: Si tiene menos de 500 (pesos o kWh), el widget podría dar un aviso visual
    const isLow = amount < 500; 
    ...
    return (
      ...
      } ${isLow && isGreen ? 'ring-4 ring-yellow-400/50' : ''}`}>
        
        {/* Decoración de fondo */}
        <div className="absolute -top-12 -right-12 w-40 h-40 bg-white/10 rounded-full blur-3xl" />
        
        <div className="relative z-10">
          <p className="text-xs font-black uppercase tracking-[0.2em] opacity-80">Saldo Disponible</p>
          ...
          <div className="flex items-baseline gap-2 mt-4">
            ...
            {/* Si usas kWh podrías ponerlo aquí al lado en pequeño */}
          </div>
          ...
          <div className="mt-10 flex items-center justify-between">
            ...
            {isLow && isGreen && (
              <span className="text-[10px] bg-yellow-400 text-black font-black px-3 py-1 rounded-full animate-bounce">
                SALDO BAJO
              </span>
            )}
          </div>
